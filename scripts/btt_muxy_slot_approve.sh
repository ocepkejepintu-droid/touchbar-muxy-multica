#!/bin/sh
# scripts/btt_muxy_slot_approve.sh — send Enter to a Touch Bar slot's tmux pane.
#
# Usage:
#   scripts/btt_muxy_slot_approve.sh <slot_index|pane_id>    # send Enter to that pane
#   scripts/btt_muxy_slot_approve.sh --self-test             # verify tmux send-keys, exit 0
#
...
# out-of-range slot → exit 0; send-keys failure → logged to daemon.log.
#
# AC-7 hold-2s approve: this script is fired by BTT Trigger Condition
# "Long Press 2s" on waiting-state slots. Tap-only path (focus.sh) emits
# no tmux send-keys call, so the Enter is gated on the 2s hold.
...
approve_pane() {
    ...
    if ! command -v tmux >/dev/null 2>&1; then
        printf 'tmux not installed; cannot send Enter to pane %s\n' "$pane_id" >&2
        log_action "FAIL tmux-not-installed pane=$pane_id"
        return 0
    fi

    if ! tmux has-session 2>/dev/null; then
        printf 'no tmux server running; cannot send Enter to pane %s\n' "$pane_id" >&2
    ...
    if tmux send-keys -t "$pane_id" Enter 2>/dev/null; then
        ...
        log_action "FAIL send-keys pane=$pane_id"
    fi
    return 0
}
...
self_test() {
    ...
    # Verify tmux send-keys is callable. tmux send-keys with no args prints
    # usage to stderr and exits 1; the binary still loaded correctly. Use
    # `tmux send-keys ''` against a known session's first pane if available,
    # otherwise fall back to checking the binary version + subcommand list.
    if tmux list-commands 2>/dev/null | grep -q '^send-keys\b'; then
        printf 'tmux send-keys: OK (binary callable, version=%s)\n' "$(tmux -V)"
    else
        printf 'tmux send-keys: UNUSABLE (binary error)\n'
    fi
    return 0
}
